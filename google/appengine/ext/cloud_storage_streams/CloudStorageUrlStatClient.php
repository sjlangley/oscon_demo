...
final class CloudStorageUrlStatClient extends CloudStorageClient {
      ...
      while (!isset($mode)) {
        ...
        foreach ($results as $result) {
          if ($result['name'] === $prefix) {
            ...
            $mode = parent::S_IFDIR;
            $mtime = $this->findLargestModifiedTime($results);
            break;
          }
        }
      }
  ...
  /**
   * Find the largest 'mtime' value in the result array.
   * Technically mtime for a directory should be the mtime of the most recently
   * modified file in the directory, but it is inefficient to try and discover
   * it, so we'll get the largest mtime in the result set that we have. This is
   * not deterministic if the directory holds > MAX_KEYS items, but it is better
   * than returning a mtime of 0.
   */
  private function findLargestModifiedTime($result_array) {
    $max = 0;
    foreach($result_array as $result) {
      $max = max($max, $result['mtime']);
    }
    return $max;
  }
}
