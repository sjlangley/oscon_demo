...
// Mock Memcache class
class Memcache {
  // Array of results to return for calls to 'get'
  static $result_array = [];

  /**
   * Poor mans mocking of results from memcache. Will return the results
   * in order frin $result_array or false if $result_array is empty.
   */
  public static function addExpectedGetResult($result) {
    self::$result_array[] = $result;
  }

  public function get($keys, $flags = null) {
    if (!empty(self::$result_array)) {
      return array_shift(self::$result_array);
    }
    return False;
  }
...
class Memcached {
  static $expected_calls = [];

  public static function addExpectedDelete($keys) {
    self::$expected_calls[] = $keys;
  }

  public static function checkExpectedDeleteCalled() {
    if (!empty(self::$expected_calls)) {
      throw new \Exception('Not all delete keys have been used, remaining: ' .
                           implode(',', $keys));
    }
  }
  ...
  public function deleteMulti($keys, $time = 0) {
    if (empty(self::$expected_calls)) {
      throw new \Exception(
          'Unexpected deleteMulti call with keys ' . implode(',', $keys));
    }
    $expected = array_shift(self::$expected_calls);
    if ($expected !== $keys) {
      throw new \Exception(
        'Keys do not match: Expected: ' . implode(',', $keys) .
        ' Actual: ' . implode(',', $expected));
    }
  }
}
...
class CloudStorageStreamWrapperTest extends ApiProxyTestBase {
  ...
  public function testReadObjectCacheHitSuccess() {
    ...
    \Memcache::addExpectedGetResult(false);
    ...
    \Memcache::addExpectedGetResult($response);
  ...
  public function testWriteLargeObjectSuccess() {
    ...
    \Memcached::addExpectedDelete($delete_keys);
